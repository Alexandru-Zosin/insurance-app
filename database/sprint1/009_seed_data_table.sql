...
VALUES
    ...
    -- France
    (@countryIdFra, 'Ile-de-France'), (@countryIdFra, 'Provence-Alpes-Cote d''Azur'), (@countryIdFra, 'Auvergne-Rhone-Alpes'),
    (@countryIdFra, 'Nouvelle-Aquitaine'), (@countryIdFra, 'Occitanie'), (@countryIdFra, 'Bretagne'),
    (@countryIdFra, 'Normandie'), (@countryIdFra, 'Hauts-de-France'), (@countryIdFra, 'Grand Est'),
    (@countryIdFra, 'Pays de la Loire'),
    -- Germany
    (@countryIdGer, 'Bavaria'), (@countryIdGer, 'Berlin'), (@countryIdGer, 'Saxony'), (@countryIdGer, 'Hesse'),
    (@countryIdGer, 'North Rhine-Westphalia'), (@countryIdGer, 'Hamburg'), (@countryIdGer, 'Baden-Wurttemberg'),
    (@countryIdGer, 'Brandenburg'), (@countryIdGer, 'Lower Saxony'), (@countryIdGer, 'Saarland');
...
WHILE @i <= 20
BEGIN
    INSERT INTO core.Client (ClientKey, ClientType, Name, RegistrationNumber, Email, Phone, Address)
    VALUES (
        NEWID(),
        CASE WHEN @i % 2 = 0 THEN 'Company' ELSE 'Person' END,
        CONCAT('Client_', @i),
        CONCAT('REG', 1000 + @i),
        CONCAT('client', @i, '@example.com'),
        CONCAT('+40123', FORMAT(@i, '00')),
        CONCAT('Str. Example nr. ', @i)
    )
    SET @i += 1
END

-- Buildings
DECLARE @clientId INT, @cityCount INT, @j INT, @cityId INT
DECLARE client_cursor CURSOR FOR SELECT ClientId FROM core.Client
OPEN client_cursor
FETCH NEXT FROM client_cursor INTO @clientId
WHILE @@FETCH_STATUS = 0
BEGIN
    SET @j = 0
    WHILE @j < (1 + ABS(CHECKSUM(NEWID())) % 3) -- 1 to 3 buildings per client
    BEGIN
        SELECT TOP 1 @cityId = CityId FROM core.City ORDER BY NEWID()
        INSERT INTO core.Building (
            BuildingKey, ClientId, CityId, ConstructionYear, Address, BuildingType, NumberOfFloors,
            SurfaceArea, InsuredValue, FloodRiskZone, EarthquakeRiskZone, InsuredValueCurrency)
        VALUES (
            NEWID(), @clientId, @cityId, 1980 + (@j * 5),
            CONCAT('Building_', @clientId, '_', @j),
            CASE WHEN @j % 3 = 0 THEN 'Residential' WHEN @j % 3 = 1 THEN 'Office' ELSE 'Industrial' END,
            1 + (@j % 5), 100 + (@j * 20), 50000 + (@j * 10000),
            NULL, NULL, 'EUR'
        )
        SET @j += 1
    END
...
INSERT INTO core.Policy (
    PolicyKey, ClientId, BuildingId, BrokerId, PremiumAmount, PremiumCurrency, StartDate, EndDate)
SELECT
    NEWID(),
    b.ClientId,
    b.BuildingId,
